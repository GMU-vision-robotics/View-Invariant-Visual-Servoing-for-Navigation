...
from utils_ddpg import buildActionMapper, update_current_pose, OUNoise
from dqn_vs import DDPG_overlap
import torch
...
#actual_episodes=2
	...
	agent = DDPG_overlap(trained_model_path=None, num_actions=action_space, input_channels=2)
	#agent = DDPG_overlap(trained_model_path=model_weights_save_path, num_actions=action_space, input_channels=2)
	noise = OUNoise(action_space=action_space)
		...
		#for point_idx in range(0, 1):
			...
			#for right_img_idx in range(3, 4):
				...
				for i_step in range(seq_len):
					...
					#action = noise.select_action(action, count_step)
					print('action = {}'.format(action))
					
					## update current_pose
					#vz, omegay = action_table[action]
					omegay = action.tolist()[0]
					print('omegay = {:.2f}'.format(omegay))
					vx = 0.0
					vz = 0.1
				...
				if right_img_idx % 10 == 0:
					...
					torch.save(agent.actor.state_dict(), '{}/actor_epoch_200000_{}.pt'.format(model_weights_save_path, scene_name))
					torch.save(agent.actor.state_dict(), '{}/actor_epoch_200000.pt'.format(model_weights_save_path))
					torch.save(agent.critic.state_dict(), '{}/critic_epoch_200000_{}.pt'.format(model_weights_save_path, scene_name))
					torch.save(agent.critic.state_dict(), '{}/critic_epoch_200000.pt'.format(model_weights_save_path))
