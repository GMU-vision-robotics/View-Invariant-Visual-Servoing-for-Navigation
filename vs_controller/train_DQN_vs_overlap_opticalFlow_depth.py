...
from util_vscontroller import genGtDenseCorrespondenseFlowMap, normalize_opticalFlow, normalize_depth, genGtDenseCorrespondenseFlowMapAndMask
from utils_ddpg import buildActionMapper, update_current_pose
from dqn_vs import DQN_vs_overlap, DQN_vs_siamese_fusion
import torch
...
def main(scene_idx=0, actual_episodes=1):
	...
	#agent = DQN_vs_siamese_fusion(trained_model_path=None, num_actions=action_space, input_channels=3)
	agent = DQN_vs_siamese_fusion(trained_model_path=model_weights_save_path, num_actions=action_space, input_channels=3)
	...
	for i_epoch in range(actual_episodes):
		...
		#for point_idx in range(0, 1):
			...
			#for right_img_idx in range(3, 4):
				...
				'''
				opticalFlow, mask_flow = genGtDenseCorrespondenseFlowMapAndMask(start_depth, goal_depth, start_pose, goal_pose)
				opticalFlow = opticalFlow[:, :, :2]
				normalized_depth = start_depth * mask_flow
				#normalized_opticalFlow = normalize_opticalFlow(opticalFlow)
				normalized_depth = normalize_depth(normalized_depth)
				'''
				opticalFlow = genGtDenseCorrespondenseFlowMap(start_depth, goal_depth, start_pose, goal_pose)
				opticalFlow = opticalFlow[:, :, :2]
				normalized_depth = start_depth.copy()
				normalized_depth = normalize_depth(normalized_depth)
				'''
				plt.imshow(opticalFlow[:,:,0])
				plt.colorbar()
				plt.show()
				plt.imshow(normalized_depth.squeeze(2))
				plt.colorbar()
				plt.show()
				'''
				...
				for i_step in range(seq_len):
					...
					'''
					new_opticalFlow, new_mask_flow = genGtDenseCorrespondenseFlowMapAndMask(next_left_depth, goal_depth, current_pose, goal_pose)
					new_opticalFlow = new_opticalFlow[:, :, :2]
					normalized_next_left_depth = next_left_depth * new_mask_flow
					#normalized_newOpticalFlow = normalize_opticalFlow(new_opticalFlow)
					normalized_next_left_depth = normalize_depth(normalized_next_left_depth)
					'''
					new_opticalFlow = genGtDenseCorrespondenseFlowMap(next_left_depth, goal_depth, current_pose, goal_pose)
					new_opticalFlow = new_opticalFlow[:, :, :2]
					normalized_next_left_depth = normalize_depth(next_left_depth)

					'''
					plt.imshow(new_opticalFlow[:,:,0])
					plt.colorbar()
					plt.show()
					plt.imshow(normalized_next_left_depth.squeeze(2))
					plt.colorbar()
					plt.show()
					'''
					#assert 1==2
					new_overlapArea = np.concatenate((new_opticalFlow, normalized_next_left_depth), axis=2)
					...
					show_opticalFlow = np.concatenate((normalized_newOpticalFlow, np.zeros((256, 256, 1), dtype=np.float32)), axis=2)
					ax.imshow(show_opticalFlow)
					ax = fig.add_subplot(r, c, 4)
					show_depth = np.squeeze(normalized_next_left_depth, axis=2)
					ax.imshow(show_depth)
					plt.show()
					assert 1==2
					'''
