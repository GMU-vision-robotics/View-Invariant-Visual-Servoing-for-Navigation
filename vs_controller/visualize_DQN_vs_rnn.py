...
from util_vscontroller import gt_goToPose, genOverlapAreaOnCurrentView, genOverlapAreaOnGoalView, genGtDenseCorrespondenseFlowMap, normalize_opticalFlow, normalize_depth
...
approach = 'twentysixth_try_opticalFlow_rnn'
mode = 'Test'
input_type = 'optical_flow_memory'#'optical_flow' #'optical_flow_depth_normalized' #'optical_flow' #'single' #'siamese' #'both'
...
perception = Perception_overlap_recurrent(2).to(device)
model = DQN_OVERLAP_Recurrent_Controller(perception, num_actions, input_size=256).to(device)
...
for point_idx in range(a, b):
	...
	#for right_img_idx in range(3, 4):
		...
		for i_step in range(seq_len):
			...
			visualized_cell_state = cell_state[0,0].reshape(16, 16).detach().cpu().numpy()
			ax.imshow(visualized_cell_state, cmap='viridis')
			ax.title.set_text('{}'.format('ct'))
