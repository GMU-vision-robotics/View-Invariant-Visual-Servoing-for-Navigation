...
approach = 'twentieth_try_triplet_with_depth' #'eighteenth_try_siamese' #'nineteenth_try_corresMapCurrentView_goToPose_metric'
mode = 'Test'
input_type = 'triplet' #'siamese' #'single' #'siamese'
...
elif input_type == 'triplet':
	perception = Perception_triplet(5).to(device)
else:
...
if input_type == 'siamese':
	...
	model = DQN_OVERLAP_Controller(perception, num_actions, input_size=768).to(device)
else:
	model = DQN_OVERLAP_Controller(perception, num_actions, input_size=256).to(device)
model.load_state_dict(torch.load('{}/dqn_epoch_{}_Collierville.pt'.format(model_weights_save_path, num_epochs)))
	...
	#for right_img_idx in range(3, 4):
		...
		for i_step in range(seq_len):
			if input_type == 'both' or input_type == 'siamese':
				...
				overlapArea_currentView = genOverlapAreaOnCurrentView(current_depth, goal_depth, current_pose, goal_pose)[:,:,:2]
				overlapArea_goalView = genOverlapAreaOnGoalView(current_depth, goal_depth, current_pose, goal_pose)[:,:,:2]
				overlapArea = np.concatenate((overlapArea_currentView, overlapArea_goalView, current_depth), axis=2)				
			else:
