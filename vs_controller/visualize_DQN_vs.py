...
from util_visual_servoing import get_train_test_scenes, get_mapper, get_mapper_scene2points, create_folder, get_mapper_dist_theta_heading, get_pose_from_name, sample_gt_random_dense_correspondences, sample_gt_dense_correspondences, get_mapper_scene2points_longer
import rrt
import random
...
if mode == 'Test':
	...
	base_folder = '/home/reza/Datasets/GibsonEnv/my_code/visual_servoing/sample_image_pairs_test_longer_dist'
...
for point_idx in range(0, 1):
	...
	for right_img_idx in range(1, len(point_pose_npy_file)):
		...
		for i_step in range(seq_len):
			...
			#unsmoothed_optical_flow = optical_flow.copy()
			optical_flow = removeCorrespondenceRandomly_withSmoothing(optical_flow, keep_prob=1.0)
			...
			if input_type == 'both':
				...
				goal_mask = np.concatenate((overlapArea_goalView, np.zeros((256, 256, 1), dtype=np.uint8)), axis=2)
				ax.imshow(goal_mask)
				...
				ft = model.perception(tensor_left)
				ft1 = ft[0, :256].reshape(1, 16, 16).detach().cpu().numpy()
				ft2 = ft[0, 256:512].reshape(1, 16, 16).detach().cpu().numpy()
				...
				#ax.title.set_text('step {}, row = {}, col = {}, Vz = {:.2f}, OmegaY= {:.2f}'.format(count_steps, y_coord, x_coord, vz, omegay))
				ax = fig.add_subplot(r, c, 6)
				im = ax.imshow(ft1[0], cmap='viridis')
				ax.title.set_text('{}'.format('ft'))
				ax.axis('off')
				ax = fig.add_subplot(r, c, 7)
				im = ax.imshow(ft2[0], cmap='viridis')
				ax.title.set_text('{}'.format('ft'))
				ax.axis('off')
			
			elif input_type == 'optical_flow_smoothed':
				ft = model.perception(tensor_left).reshape(1, 16, 16).detach().cpu().numpy()
				fig = plt.figure(figsize=(20, 20)) #cols, rows
				fig.tight_layout()
				r, c = 3, 4
				ax = fig.add_subplot(r, c, 1)
				ax.imshow(current_img)
				...
				ax.imshow(Qvalue_table_np, cmap='hot')
				#ax.axis('off')
				ax = fig.add_subplot(r, c, 9)
				x_axis_flow = unsmoothed_optical_flow[:, :, 0]
				im1 = ax.imshow(x_axis_flow, vmin=-100, vmax=100, cmap='viridis')
				fig.colorbar(im1, fraction=0.046, pad=0.04)
				ax.axis('off')
				ax = fig.add_subplot(r, c, 10)
				y_axis_flow = unsmoothed_optical_flow[:, :, 1]
				im = ax.imshow(y_axis_flow, vmin=-100, vmax=100, cmap='viridis')
				fig.colorbar(im, fraction=0.046, pad=0.04)
				ax.axis('off')
				tensor_right = torch.tensor(unsmoothed_optical_flow, dtype=torch.float32).to(device).unsqueeze(0).permute(0, 3, 1, 2)
				ft_right = model.perception(tensor_right).reshape(1, 16, 16).detach().cpu().numpy()
				Qvalue_table_right = model(tensor_right)
				ax = fig.add_subplot(r, c, 11)
				ax.imshow(ft_right[0], cmap='viridis')
				ax.axis('off')
				ax = fig.add_subplot(r, c, 12)
				Qvalue_table_right_np = np.squeeze(Qvalue_table_right.detach().cpu().numpy(), axis=0).reshape((1, 7))
				ax.imshow(Qvalue_table_right_np, cmap='hot')

			elif input_type == 'optical_flow':
				ft = model.perception(tensor_left).reshape(1, 16, 16).detach().cpu().numpy()
				fig = plt.figure(figsize=(20, 12)) #cols, rows
				fig.tight_layout()
				r, c = 2, 3
				ax = fig.add_subplot(r, c, 2)
				ax.imshow(current_img)
				ax.axis('off')
				ax.title.set_text('{}'.format('Current View'))
				ax = fig.add_subplot(r, c, 1)
				ax.imshow(goal_img)
				ax.axis('off')
				ax.title.set_text('{}'.format('Target View'))
				ax = fig.add_subplot(r, c, 3)
				Qvalue_table_np = np.squeeze(Qvalue_table.detach().cpu().numpy(), axis=0).reshape((1, 7))
				ax.imshow(Qvalue_table_np, cmap='hot')
				ax.title.set_text('{}'.format('Q-value Table'))
				ax = fig.add_subplot(r, c, 4)
				x_axis_flow = optical_flow[:, :, 0]
				im1 = ax.imshow(x_axis_flow, vmin=-100, vmax=100, cmap='viridis')
				fig.colorbar(im1, fraction=0.046, pad=0.04)
				ax.axis('off')
				ax.title.set_text('{}'.format('x-axis corrMap'))
				ax = fig.add_subplot(r, c, 5)
				y_axis_flow = optical_flow[:, :, 1]
				im = ax.imshow(y_axis_flow, vmin=-100, vmax=100, cmap='viridis')
				fig.colorbar(im, fraction=0.046, pad=0.04)
				ax.axis('off')
				ax.title.set_text('{}'.format('y-axis corrMap'))
				ax = fig.add_subplot(r, c, 6)
				im = ax.imshow(ft[0], cmap='viridis')
				ax.axis('off')
				ax.title.set_text('{}'.format('feature map'))
				#ax.title.set_text('step {}, row = {}, c
				...
				ax.imshow(goal_img)
				ax = fig.add_subplot(r, c, 3)
				visualized_optical_flow = np.concatenate((normalized_opticalFlow, np.zeros((256, 256, 1), dtype=np.float32)), axis=2)
				ax.imshow(visualized_optical_flow)
				ax = fig.add_subplot(r, c, 4)
				ax.imshow(normalized_depth.squeeze(2))
				ax = fig.add_subplot(r, c, 5)
				Qvalue_table_np = np.squeeze(Qvalue_table.detach().cpu().numpy(), axis=0).reshape((1, 7))
				ax.imshow(Qvalue_table_np, cmap='hot')
				#ax.title.set_text('step {}, row = {}, col = {}, Vz = {:.2f}, OmegaY= {:.2f}'.format(count_steps, y_coord, x_coord, vz, omegay))
				...
				ft = model.perception(tensor_left).reshape(2, 256)
				ft_0 = ft[0, :].reshape(16, 16).detach().cpu().numpy()
				ft_1 = ft[1, :].reshape(16, 16).detach().cpu().numpy()
				fig = plt.figure(figsize=(25, 6)) #cols, rows
				r, c = 1, 9
				ax = fig.add_subplot(r, c, 1)
				ax.imshow(current_img)
				...
				ax.imshow(goal_img)
				ax = fig.add_subplot(r, c, 3)
				x_axis_flow = old_opticalFlow[:, :, 0]
				ax.imshow(x_axis_flow)
				ax = fig.add_subplot(r, c, 4)
				y_axis_flow = old_opticalFlow[:, :, 1]
				ax.imshow(y_axis_flow)
				ax = fig.add_subplot(r, c, 5)
				x_axis_flow = optical_flow[:, :, 0]
				ax.imshow(x_axis_flow)
				ax = fig.add_subplot(r, c, 6)
				...
				#ax.title.set_text('step {}, row = {}, col = {}, Vz = {:.2f}, OmegaY= {:.2f}'.format(count_steps, y_coord, x_coord, vz, omegay))
				ax = fig.add_subplot(r, c, 8)
				im = ax.imshow(ft_0, cmap='viridis')
				ax.title.set_text('{}'.format('ft'))
				ax = fig.add_subplot(r, c, 9)
				im = ax.imshow(ft_1, cmap='viridis')
				ax.title.set_text('{}'.format('ft'))
				ax.axis('off')
			else:
				ft = model.perception(tensor_left).reshape(1, 16, 16).detach().cpu().numpy()
