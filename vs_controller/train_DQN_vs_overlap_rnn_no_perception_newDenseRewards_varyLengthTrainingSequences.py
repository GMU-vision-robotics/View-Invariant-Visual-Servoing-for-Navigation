...
from util_vscontroller import gt_goToPose, genGtDenseCorrespondenseFlowMap
from utils_ddpg import buildActionMapper, update_current_pose
from model_vs import *
#from dqn_vs import DQN_vs_overlap_rnn, DQN_vs_overlap_rnn_no_perception
import torch
...
from model_vs import *
from util_vscontroller import ReplayMemory_vs_dqn, ReplayMemory_overlap_dqn, ReplayMemory_overlap_dqn_recurrent, ReplayMemory_overlap_dqn_episodes
import random
import numpy as np
import math
...
class DQN_vs_overlap_rnn_no_perception_new:
    def __init__(self, trained_model_path=None, num_actions=2, input_channels=2, actor_learning_rate=1e-5, critic_learning_rate=1e-4, gamma=0.97, tau=1e-2, max_memory_size=1000):
        ...
        # Networks
        self.actor = DQN_OVERLAP_Recurrent_Controller_no_perception_dense(self.num_actions, input_size=256).to(device)
        self.critic = DQN_OVERLAP_Recurrent_Controller_no_perception_dense(self.num_actions, input_size=256).to(device)
        ...
        # Training
        self.memory = ReplayMemory_overlap_dqn_episodes(max_memory_size)        
    ...
    def select_action(self, state, hidden_state, EPS_START=0.9, EPS_END=0.05, EPS_DECAY=10000):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                ...
                #print('obs.shape = {}'.format(obs.shape))
                #model_out = self.actor.forward(obs, hidden_state, cell_state, batch_size=1)
                model_out = self.actor.forward(obs, hidden_state, batch_size=1)
                #print('model_out[0].shape = {}'.format(model_out[0].shape))
                action = model_out[0].squeeze(1).max(1)[1].view(1, 1)
    ...
    def update(self, batch_size):
        ...
        seq_lengths = torch.LongTensor([len(seq) for seq in batches]).cuda()
        #print('seq_lengths = {}'.format(seq_lengths))

        #print('batches.shape = {}'.format(batches.shape))
        max_seq_length = seq_lengths.max()
        states = torch.zeros((batch_size, max_seq_length, 256), dtype=torch.float32).to(device)
        actions = torch.zeros((batch_size, max_seq_length), dtype=torch.long).to(device)
        rewards = torch.zeros((batch_size, max_seq_length), dtype=torch.float32).to(device)
        next_states = torch.zeros((batch_size, max_seq_length, 256), dtype=torch.float32).to(device)
        done = torch.zeros((batch_size, max_seq_length), dtype=torch.float32).to(device)
        ...
        for i, b in enumerate(batches):
            ...
            for j, elem in enumerate(b):
                ...
                previous_pose = elem[2][0]
                current_pose = elem[2][1]
                current_reward, _, _ = decide_reward_and_done(previous_pose, current_pose, goal_pose, start_pose)
                next_states[i, j] = torch.tensor(elem[3], dtype=torch.float32)
                rw.append(current_reward)
                do.append(float(elem[4]))
                #do.append(elem[4])
            actions[i, :seq_lengths[i]] = torch.tensor(ac, dtype=torch.long)
            rewards[i, :seq_lengths[i]] = torch.tensor(rw, dtype=torch.float32)
            done[i, :seq_lengths[i]] = torch.tensor(do, dtype=torch.float32)
        ...
        #Qvals, _ = self.actor.forward_packed(states, hidden_batch, cell_batch, seq_lengths.cpu().numpy()) ## batch_size x action_space
        Qvals, _ = self.actor.forward_packed(states, hidden_batch, seq_lengths.cpu().numpy()) ## batch_size x action_space
        #print('Qvals.shape = {}'.format(Qvals.shape))
        #print('Qvals = {}'.format(Qvals))
        Qvals = Qvals.gather(2, actions.unsqueeze(2)).squeeze(2) ## [batch_size, timestep]
        #print('Qvals.shape = {}'.format(Qvals.shape))
        #print('Qvals = {}'.format(Qvals))
        #hidden_batch, cell_batch = self.critic.init_hidden_states(batch_size=batch_size)
        #next_Q, _ = self.critic.forward_packed(next_states, hidden_batch, cell_batch, seq_lengths.cpu().numpy())##batch_size x action_space
        hidden_batch = self.actor.init_hidden_states(batch_size=batch_size)
        next_Q, _ = self.critic.forward_packed(next_states, hidden_batch, seq_lengths.cpu().numpy())##batch_size x action_space
        #print('next_Q.shape = {}'.format(next_Q.shape))
        #print('next_Q = {}'.format(next_Q))
        next_Q = next_Q.max(2)[0].detach() ## batch_size x timestep
        #print('next_Q.shape = {}'.format(next_Q.shape))
        #print('next_Q = {}'.format(next_Q))
        ...
        for i in range(batch_size):
            loss += F.smooth_l1_loss(Qvals[i, :seq_lengths[i]], Qprime[i, :seq_lengths[i]])
	...
	#for point_idx in range(0, 1):
		...
		#for right_img_idx in range(3, 4):
			...
			for i_step in range(seq_len):
				...
				local_memory.append((state, action, (previous_pose, current_pose, goal_pose, start_pose), new_state, done))
				#assert 1==2
				
				if len(agent.memory) >= batch_size:
					agent.update(batch_size)
				elif len(agent.memory) >= 3:
					agent.update(len(agent.memory))
			...
			agent.memory.push(local_memory)
...
if __name__ == "__main__":
	...
	main(args.scene_idx, args.epoch)
'''