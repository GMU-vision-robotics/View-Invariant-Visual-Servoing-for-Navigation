...
from dqn_vs import DQN_vs_siamese
import torch
...
#actual_episodes=2
	...
	#agent = DQN_vs_siamese(trained_model_path=None, num_actions=action_space, input_channels=4)
	agent = DQN_vs_siamese(trained_model_path=model_weights_save_path, num_actions=action_space, input_channels=4)
		...
		#for point_idx in range(0, 1):
			...
			#for right_img_idx in range(3, 4):
				...
				for i_step in range(seq_len):
					...
					new_overlapArea = np.concatenate((new_overlapArea_currentView, new_overlapArea_goalView), axis=2)
