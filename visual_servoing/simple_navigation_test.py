...
presampled_poses = [current_pose]
for i in range(3):
	x, y, theta = presampled_poses[-1]
	#x = x + 0.1 * cos(theta)
	#y = y + 0.1 * sin(theta)
	theta = theta + math.pi/12
	new_pose = [x, y, theta]
	presampled_poses.append(new_pose)
...
while count_steps < seq_len:
	...
	plt.savefig('{}/step_{}.jpg'.format(file_addr, count_steps), bbox_inches='tight')
	plt.close()
